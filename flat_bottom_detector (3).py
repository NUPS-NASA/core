...
class DetectionParams:
    ...
    # scope for edge-monotone (only trigger when plateau is near edges)
...
def _edge_monotone_reject(
    ...
    inside_med = float(np.median(y_s[s : e + 1]))
    # scope: only consider edge-monotone if plateau boundary is close to the edge
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--edge-monotone-scope-pts", type=int, default=12, help="Only apply edge-monotone check if plateau boundary is within this many points from the edge")
