...
"""
U/V dip detector with:
- Coarse grid search + optional JAX refine
- Rectangle/bottom-fill rejection (global + local)
- Curvature/slope/symmetry/horizontal-coverage checks
- Quadratic trend (sky flux) residual guard inside interval
- Optional "refine-only" mode with strict acceptance of refined result
"""
...
def robust_quadratic_trend(t: np.ndarray, y: np.ndarray, iters:int=6) -> dict:
    """
    Robustly fit a quadratic trend y ≈ b0 + b1*x + b2*x^2 using IRLS with Huber weights.
    x is normalized to [-1,1] for numerical stability.
    Returns dict with keys: beta (3,), yhat (N,), resid (N,), sigma (MAD of resid), x (N,).
    """
    tmin=float(np.min(t)); tmax=float(np.max(t)); span=max(tmax-tmin, 1e-9)
    x = 2.0 * (t - (tmin + 0.5*span)) / span  # [-1, 1]
    X = np.vstack([np.ones_like(x), x, x*x]).T
    # Initial OLS
    try:
        beta, *_ = np.linalg.lstsq(X, y, rcond=None)
    except Exception:
        beta = np.array([np.median(y), 0.0, 0.0], dtype=float)
    yhat = X @ beta
    resid = y - yhat
    def mad(z): return 1.4826 * (np.median(np.abs(z - np.median(z))) + 1e-12)
    sigma = mad(resid); delta = 1.345 * sigma if sigma>0 else 1.0
    for _ in range(iters):
        r = y - (X @ beta)
        sigma = mad(r)
        delta = 1.345 * sigma if sigma>0 else 1.0
        a = np.abs(r); w = np.ones_like(a)
        mask = a > delta
    ...
    resid = y - yhat
    sigma = mad(resid)
    return {"beta": beta, "yhat": yhat, "resid": resid, "sigma": sigma, "x": x}
...
def coarse_grid_search(t: np.ndarray, y: np.ndarray, centers:int=80, widths:int=25, depths:int=40)->Dict[str,float]:
    ...
    return {"a": best["a"], "center": best["center"], "width": best["width"], "depth": best["depth"],
            "improvement": float(improvement), "snr": float(snr), "loss_base": float(loss_base), "loss_model": float(best["loss"]), "tau": float(tspan*0.01)}
...
def uv_shape_checks(t: np.ndarray, y: np.ndarray, c: float, w: float,
                    ...
                    resid_all: Optional[np.ndarray], sigma_resid: float,
                    trend_guard: bool, trend_k_up: float, trend_k_low: float, trend_max_above: float, trend_min_below: float, trend_med_k: float,
    ...
    if len(yV)==0:
        return {"shape_ok": True, "is_rect": is_rect, "z_drop": 0.0, "q3_ok": True, "lower_rect_like": False, "slope_ok": True,
                "rect_score": rect_score, "sym_ok": True, "hcov_ok": True, "gap_ok": True, "edge_ok": True, "curv_ok": True, "lower_band_dom": False, "trend_ok": True, "trend_stats": {"frac_above":0.0,"frac_below":0.0}}
    ...
    # Trend (sky flux) guard: residual balance inside interval
    trend_ok = True
    trend_med_k = float(trend_med_k)
    trend_stats = {"frac_above": 0.0, "frac_below": 0.0}
    if trend_guard and (sigma_resid > 0) and (resid_all is not None):
        r_in = resid_all[inside_mask]
        if r_in.size >= 10:
            frac_above = float(np.mean(r_in >= (trend_k_up * sigma_resid)))
            frac_below = float(np.mean(r_in <= (-trend_k_low * sigma_resid)))
            trend_stats = {"frac_above": frac_above, "frac_below": frac_below}
            med = float(np.median(r_in)) / float(sigma_resid + 1e-12)
            trend_ok = (frac_above <= trend_max_above) and (frac_below >= trend_min_below) and (med <= float(-1.0*trend_med_k))

    # Borderline allowance
    borderline = (z_drop < (z_drop_thr + 0.6)) and q3_ok and sym_ok
    shape_ok = shape_basic and hcov_ok and trend_ok and ( (sym_ok and (gap_ok or curv_ok)) or (z_drop >= (z_drop_thr+1.0)) or slope_ok or borderline )
    print({"shape_basic": shape_basic, "hcov_ok": hcov_ok, "trend_ok": trend_ok, "sym_ok": sym_ok, "gap_ok": gap_ok, "curv_ok": curv_ok, "z_drop": z_drop, "slope_ok": slope_ok, "borderline": borderline})
    return {"shape_ok": bool(shape_ok), "is_rect": bool(is_rect), "z_drop": float(z_drop), "q3_ok": bool(q3_ok),
            ...
            "curv_ok": bool(curv_ok), "lower_band_dom": bool(lower_band_dom), "trend_ok": bool(trend_ok), "trend_stats": trend_stats}
...
def detect_on_xy(t: np.ndarray, y: np.ndarray,
                 ...
                 grid_only: bool=False, debug: bool=False,
                 refine_only: bool=False, refine_min_gain: float=0.02, refine_min_support: int=24,
                 trend_guard: bool=False, trend_k_up: float=0.9, trend_k_low: float=0.6, trend_max_above: float=0.28, trend_min_below: float=0.34, trend_med_k: float=0.35)->Dict[str,Any]:

    mask=np.isfinite(t)&np.isfinite(y); t=t[mask]; y=y[mask]; o=np.argsort(t); t=t[o]; y=y[o]

    # Global quadratic trend residuals
    trend = robust_quadratic_trend(t, y)
    resid_all = trend["resid"]; sigma_resid = trend["sigma"]
    ...
    shape_c=uv_shape_checks(t,y,coarse["center"],coarse["width"], rect_thr, z_drop_thr, slope_sigma_factor,
                            ...
                            lower_band_k, lower_band_frac, resid_all, sigma_resid, trend_guard, trend_k_up, trend_k_low, trend_max_above, trend_min_below, trend_med_k, inside_c)
    ...
    if debug:
        print(f"[COARSE] imp={coarse['improvement']:.3f} snr={coarse['snr']:.3f} support={support_c} "
              ...
              f"trend_ok={shape_c.get('trend_ok', True)} trend={shape_c.get('trend_stats', {})}")

    if (not grid_only) and (not refine_only):
    ...
    shape_r=uv_shape_checks(t,y,refined["center"],refined["width"], rect_thr, z_drop_thr, slope_sigma_factor,
                            q3_near_baseline_sigmas, local_rect_occ_thr, slope_gap_factor, shoulder_sym_sigmas,
                            hcov_bins, hcov_min_occ, min_edge_frac, edge_override_imp, curv_sigma_factor,
                            lower_band_k, lower_band_frac, resid_all, sigma_resid, trend_guard, trend_k_up, trend_k_low, trend_max_above, trend_min_below, trend_med_k, inside)
    ...
    # Strict acceptance gate for refined solution
    refine_shape_ok = (not shape_r["is_rect"]) and shape_r["shape_ok"] and (shape_r["slope_ok"] or shape_r["curv_ok"] or (shape_r["z_drop"] >= (z_drop_thr + 0.8)))
    refine_gain_ok = (refined["improvement"] >= (coarse["improvement"] + max(0.0, refine_min_gain)))
    refine_support_ok = (support_r >= max(refine_min_support, min_points))
    refined_accept = refine_shape_ok and refine_gain_ok and refine_support_ok and (shape_r["edge_ok"] or (refined["improvement"]>=edge_override_imp))

    if refine_only:
        has_dip = has_dip and refined_accept  # require refined pass

    # Guard for FN (disabled when refine-only)
    if (not has_dip) and (not shape_r.get("is_rect", False)) and shape_r.get("q3_ok", False) \
       ...
       and (shape_r.get("z_drop", 0.0) >= z_drop_thr) and rule_or and (support_r >= guard_min_support) and (not refine_only):
        has_dip = True

    # Borderline leniency support floor
    ...
    # Fallback to coarse (unless refine-only)
    if (not has_dip) and (not refine_only):
    ...
    if has_dip:
        reason=None
    elif shape_r.get("is_rect", False):
        reason=f"Global vertical distribution looks rectangular (score={shape_r.get('rect_score',0.0):.2f})."
    elif not shape_r.get("shape_ok", False):
        reason=f"Rejected by U/V shape (z={shape_r.get('z_drop',0.0):.2f}, q3={shape_r.get('q3_ok',False)}, slope={shape_r.get('slope_ok',False)}, lower_rect={shape_r.get('lower_rect_like',False)}, trend_ok={shape_r.get('trend_ok',True)})."
    else:
    ...
    if debug:
        print(f"[REFINE={'True' if ((not grid_only) and HAVE_JAX) else 'False'}] imp={refined['improvement']:.3f} snr={refined['snr']:.3f} support={support_r} "
              f"z={shape_r.get('z_drop',0.0):.2f} q3={shape_r.get('q3_ok',False)} slope={shape_r.get('slope_ok',False)} "
              f"lower_rect={shape_r.get('lower_rect_like',False)} rect_score={shape_r.get('rect_score',0.0):.2f} "
              f"sym={shape_r.get('sym_ok',False)} hcov={shape_r.get('hcov_ok',False)} gap={shape_r.get('gap_ok',False)} curv={shape_r.get('curv_ok',False)} edge={shape_r.get('edge_ok',False)} "
              f"trend_ok={shape_r.get('trend_ok',True)} trend={shape_r.get('trend_stats',{})} "
              f"AND={rule_and} OR={rule_or} refined_accept={refined_accept} has_dip={has_dip}")

    return {"has_dip": bool(has_dip), "reason": reason, "a": refined["a"], "depth": refined["depth"], "center": refined["center"],
            "width": refined["width"], "snr": refined["snr"], "improvement": refined["improvement"], "rect_score": shape_r.get("rect_score",0.0),
            "support_points": support_r, "source": "refined" if ((not grid_only) and HAVE_JAX) else "coarse", "z_drop": shape_r.get("z_drop",0.0)}
...
def process_folder(root: Path, out_dir: Path, time_col: Optional[str], y_col: Optional[str],
                   ...
                   grid_only: bool=False, debug: bool=False,
                   refine_only: bool=False, refine_min_gain: float=0.02, refine_min_support: int=24,
                   trend_guard: bool=False, trend_k_up: float=0.9, trend_k_low: float=0.6, trend_max_above: float=0.28, trend_min_below: float=0.34, trend_med_k: float=0.35)->Path:
    ...
    for p in csv_files:
        try:
            ...
            res=detect_on_xy(t,y,prefer_sensitivity=prefer_sensitivity,imp_override=imp_override,base_snr=base_snr,base_imp=base_imp,
                             ...
                             grid_only=grid_only, debug=debug, refine_only=refine_only, refine_min_gain=refine_min_gain, refine_min_support=refine_min_support,
                             trend_guard=trend_guard, trend_k_up=trend_k_up, trend_k_low=trend_k_low, trend_max_above=trend_max_above, trend_min_below=trend_min_below, trend_med_k=trend_med_k)
...
def main():
    ...
    ap.add_argument("--curv-sigma", type=float, default=0.5, dest="curv_sigma", help="Min quadratic curvature (k) in units of sigma_shoulder/w^2.")
    ...
    ap.add_argument("--refine-only", action="store_true", help="Require refined solution to pass strict gate; do not auto-accept coarse results.")
    ap.add_argument("--refine-min-gain", type=float, default=0.02, dest="refine_min_gain", help="Min improvement gain (refined - coarse) to accept refined result.")
    ap.add_argument("--refine-min-support", type=int, default=24, dest="refine_min_support", help="Min support points inside interval to accept refined result.")
    ap.add_argument("--trend-guard", action="store_true", help="Enable quadratic trend (sky flux) guard inside interval.")
    ap.add_argument("--trend-k-up", type=float, default=0.9, dest="trend_k_up", help="Threshold (in sigma) for counting points above trend as conflicting with a dip.")
    ap.add_argument("--trend-k-low", type=float, default=0.6, dest="trend_k_low", help="Threshold (in sigma) for counting points below trend as supporting a dip.")
    ap.add_argument("--trend-max-above", type=float, default=0.28, dest="trend_max_above", help="Max frac of inside points allowed above trend by +k_up*sigma.")
    ap.add_argument("--trend-min-below", type=float, default=0.34, dest="trend_min_below", help="Min frac of inside points required below trend by -k_low*sigma.")
    ap.add_argument("--trend-med-k", type=float, default=0.35, dest="trend_med_k", help="Require median residual inside interval <= -k*sigma.")
    ...
    summary_path=process_folder(root,out_dir,args.time_col,args.y_col, prefer_sensitivity=(not args.strict),
                                ...
                                grid_only=args.grid_only, debug=args.debug, refine_only=args.refine_only, refine_min_gain=args.refine_min_gain, refine_min_support=args.refine_min_support,
                                trend_guard=args.trend_guard, trend_k_up=args.trend_k_up, trend_k_low=args.trend_k_low, trend_max_above=args.trend_max_above, trend_min_below=args.trend_min_below, trend_med_k=args.trend_med_k)
