...
# -----------------------------
# Aggressive ensemble (to reduce FN)
# -----------------------------
def _rolling_quantile(signal, win, q):
    import numpy as _np
    n = len(signal)
    if n == 0:
        return _np.array([])
    ...
    k = k + (k % 2 == 0)  # odd
    pad = k // 2
    padded = _np.pad(signal, (pad, pad), mode='edge')
    out = _np.empty(n, dtype=float)
    for i in range(n):
        out[i] = _np.quantile(padded[i:i+k], q)
    return out

def _detect_aggressive_interval(grid, lo, *, width_min, depth_thresh):
    """
    Simpler, high-recall pass: use rolling median as baseline and find
    contiguous regions where (baseline - lo) exceeds depth_thresh.
    """
    import numpy as _np
    # Rolling median baseline of q10
    base = _rolling_quantile(lo, win=max(21, int(len(lo)*0.05)), q=0.5)
    d = base - lo
    mask = d > depth_thresh
    # Find widest / deepest segment
    segs = []
    i = 0
    while i < len(mask):
        if mask[i]:
    ...
    if not segs:
        return None
    # Score by area (depth integral)
    best = None
    for (a, b) in segs:
        width = grid[b] - grid[a]
        if width < width_min:
            continue
        area = float(_np.trapz(d[a:b+1], grid[a:b+1]))
        peak = float(_np.max(d[a:b+1]))
        if (best is None) or (area > best[0]):
            best = (area, peak, a, b)
    if best is None:
        return None
    return {"a": best[2], "b": best[3], "peak": best[1]}
...
def detect_dip_interval(
    ...
    depth_k=0.08,          # threshold scale vs robust spread
    min_width_frac=0.02,   # min width vs x-range
    ...
    # Also require low variability of spread across x to call it rectangle-like
    # Compute a coarse variability proxy: MAD of (hi-lo) vs its median
    spread = hi - lo
    spread_mad = float(np.median(np.abs(spread - np.median(spread))) + 1e-12)
    rectangle_like = (spread_median > 0.8 * y_range) and (spread_mad / (spread_median + 1e-12) < 0.25)
    if rectangle_like:
        if verbose:
        ...
        if segs:
            ...
            if best is not None:
                ...
                # Flexible valley shape: ends should be much shallower than the center peak.
                peak_idx = int(a + np.argmax(d[a:b+1]))
                peak = float(d[peak_idx])
                # Relax valley constraint to reduce false negatives:
                valley_ok = True
                ...
                # Softer gate: only reject when upper drop dwarfs lower drop.
                top_missing_ok = top_ratio <= 1.25
    ...
    if debug_dump_path is not None:
        try:
            ...
            df_dbg = _pd.DataFrame(dbg)
            Path(debug_dump_path).parent.mkdir(parents=True, exist_ok=True)
            _pd.DataFrame(df_dbg).to_csv(debug_dump_path, index=False)
        except Exception as _e:
            if verbose:
                print("debug dump error:", _e)
    return {
...
def main():
    ...
    ap.add_argument("--mode", type=str, default="standard", choices=["standard","aggressive"], help="Detection mode")
    ap.add_argument("--debug_dump", type=str, default=None, help="Path to save q10/q90 grid debug CSV")
