...
# -*- coding: utf-8 -*-
"""
Flat-Bottom (Plateau-at-Minimum) Detector — from scratch (clean rebuild)

Overview
--------
Given CSV files (with headers; first column is X, Y column is a given name or
defaults to the second column), detect whether a curve has a "flat-bottom"
region — i.e., a segment that is lower than its surroundings and fairly flat —
*excluding* cases where the region starts right at the left edge or ends at the
right edge, and *rejecting* pure quadratic U-shapes.

Core ideas
----------
1) Smooth with a centered moving average (default window=20; enforced odd).
2) Model selection:
   - Fit Quadratic: y ~ ax^2 + bx + c  (explains pure U shapes well)
   - Fit Plateau Piecewise: left linear ramp + flat middle + right linear ramp.
   - Accept plateau only if piecewise SSE improves over quadratic by a margin
     (model_improve_min) AND depth vs surroundings is sufficient (model_contrast_min).
3) FP suppression via depth/shoulder filters (must all pass):
   - Bilateral depth (each side's median vs inside median in MAD units)
   - Shoulder slope directionality (left mostly negative slope, right mostly positive)
   - Inside flatness (fraction of small-slope points, MAD threshold)
   - Baseline-line depth: fit lines on left/right shoulders and require inside to
     be deeper than linear extrapolation at the middle.
4) Hard padding from both ends: the plateau must start and end with padding both
   in points and in x-distance fraction.
5) Fallback (optional): Triple-tangent rule (1st~3rd stationary points) with depth
   & padding checks, if model selection fails (enabled by default).

Outputs
-------
- out/summary.csv               : one row per file with detection info
- out/plots/.../<file>.png      : annotated plot (optional with --plot)
- out/debug_csv/.../<file>.csv  : debug arrays (optional with --export-debug-csv)
...
def moving_average_centered(y: np.ndarray, window: int) -> np.ndarray:
    """Centered moving average using reflect padding; length == len(y)."""
    N = len(y)
    if window <= 1 or N == 0:
        return y.copy()
    if window % 2 == 0:
...
class PlateauSegment:
    ...
    mean_abs_curv: float
    score: float   # interpretation: higher = better, used internally
...
class DetectionParams:
    ...
    edge_monotone_frac_min: float = 0.75   # fraction of monotone slopes from edge
    edge_drop_min: float = 0.50            # required drop from edge median to inside (MAD units)
...
def _best_plateau_piecewise(
    ...
    def refit_for_span(s: int, e: int) -> Dict[str, Any]:
        # build design matrix for piecewise: [1, left_slope_term, right_slope_term]
        rows, rhs = [], []
        if s > 0:
            xs = x[s]
            for i in range(0, s):
        ...
        return {
            "s": s, "e": e, "y0": y0, "mL": mL, "mR": mR,
            "sse": sse, "yhat": yhat, "contrast": contrast,
            "width": width, "score": score,
            "width_norm": width_norm, "sse_norm": sse_norm,
        }

    best = None

    # brute-force candidates with hard padding constraints
    for s in range(edge_margin, n - edge_margin - min_points):
        for e in range(s + min_points - 1, n - edge_margin):
            # hard padding: points and x-distance
    ...
    # greedy widening within allowed SSE tolerance
    improved = True
    while improved:
        improved = False
        # extend left
...
def _depth_shoulders_filter(
    ...
    return ok, {
        "gapL": gapL, "gapR": gapR,
        "left_neg_frac": left_neg_frac, "right_pos_frac": right_pos_frac,
    ...
    for k in range(len(stat_idx) - 2):
        ...
        candidates.append(
            PlateauSegment(
                ...
                start_x=float(x[i1]), end_x=float(x[i3]), x_span=x_span,
                mean_y=float(np.mean(inside_y)), min_y=float(np.min(inside_y)),
        ...
        details.append({
            ...
            "left_med": left_med, "right_med": right_med,
            "inside_med": inside_med, "contrast": contrast,
        })
...
best = max(candidates, key=lambda seg: seg.score)
    debug = {
        "y_s": y_s, "slope": slope, "curv": curv,
        "stationary_idx": stat_idx, "method": "triple_tangent",
        "best": asdict(best),
    }
    return best, debug
...
def detect_flat_bottom_modelselect(
    ...
    y_scale = robust_scale(y_s)
    ok, filt = _depth_shoulders_filter(
        x=x, y_s=y_s, s=best_pl["s"], e=best_pl["e"],
        slope=slope, y_scale=y_scale, params=params
    )
    if not ok:
        return None, {
    ...
    # Edge-monotone rejection: direct descent from left or ascent to right edges
    em_reject, em = _edge_monotone_reject(
        x=x, y_s=y_s, s=best_pl["s"], e=best_pl["e"],
        slope=slope, y_scale=y_scale, params=params
    )
    ...
    seg = PlateauSegment(
        ...
        start_x=float(x[s]), end_x=float(x[e]), x_span=float(x[e] - x[s]),
        mean_y=float(np.mean(seg_y)), min_y=float(np.min(seg_y)),
        max_abs_slope=float(np.max(np.abs(seg_slope))),
        mean_abs_slope=float(np.mean(np.abs(seg_slope))),
        mean_abs_curv=float(np.mean(np.abs(seg_curv))),
        score=float(improve),
...
def process_file(
    ...
    if export_debug_csv:
        ...
        debug_df = pd.DataFrame({
            "x": x, "y": y, "y_smooth": y_s,
            "slope": slope, "curv": curv,
        })
        if best is not None:
    ...
    if best is not None:
        row.update({
            "start_idx": best.start_idx, "end_idx": best.end_idx,
            "start_x": best.start_x,     "end_x": best.end_x,
            "x_span": best.x_span,       "mean_y": best.mean_y,
            "min_y": best.min_y,         "score": best.score,
        })
    else:
        row.update({
            "start_idx": None, "end_idx": None,
            "start_x": None,   "end_x": None,
            "x_span": None,    "mean_y": None,
            "min_y": None,     "score": None,
        })
    return row
...
def run(
    ...
    for p in files:
        try:
            row = process_file(
                path=p, rel_root=input_dir, out_dir=output_dir,
                y_col=y_col, params=params,
                export_debug_csv=export_debug_csv, make_plot=make_plot,
            )
        except Exception as e:
            row = {"file": str(p), "rel_file": sanitize_rel_path(input_dir, p).as_posix(),
